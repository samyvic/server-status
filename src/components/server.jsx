...
export default function Server() {
    ...
    return (
        ...
        servers.map((server,index) => {
            return <div >
                <div
                className="server p-6 my-2 mx-40 font-bold bg-white border shadow" key={index}>
                    {server.name} <span style={{ color: server.status === "Online" ? 'green' : 'red' }}>{` [ ${server.status} ] `}</span>
                    ...
                    <span className='block mt-4 text-indigo-500 cursor-pointer' 
                     onClick={() => {showDetails(index)}} >Show Details</span>
                </div>
                <div></div>
            </div>
